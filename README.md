...
| Company D, AI product with ~60 people | Zero connections. `bridge` on the hiring manager found no career overlap in the author's top 150 contacts either. But a close friend works at a company whose people the author had heard "know a lot of people there" | `relay --via friend --target D` pulled both rosters and found the friend's coworker who had spent 3+ years at a previous employer alongside D's CRO, plus two weaker relays. Also the recruiter and the growth-product lead via `discover`, each with a seat-specific draft | Three routes where there had been zero: a warm relay through the friend, a recruiter state-check, and a product-feedback note to the growth lead. Relay drafted first; outcome logged when known. |

A fifth, smaller one: a peer at a startup the author had just applied to and did not know how to message without it reading as "hey I applied, let's chat." The `cold` peer shape (specific reason, real question, one-clause disclosure, small ask) is what went out.

Case D is the one that made the second-degree layer earn its place. The mutual-connections list on the hiring manager's profile had 80-odd names and, by the author's own guess, two or three real relationships among them; nobody was going to read it. The relay came from work history, not from that list, and it named a specific person and a specific reason. Whether it converts is logged in `outcomes`, and this table will be updated honestly, including the misses.
...
LinkedIn export (zip)                    Exa people index (optional)
    ...
    relay.py    third degree: my contact -> their coworker -> the target, from two rosters
        |
